...
function renderNode(key, value, path, root = false) {
  ...
  if (value && typeof value === 'object') {
    const keys = Object.keys(value).filter(child => !child.startsWith('_'));
...
function renderUpload(path) {
  return `<div class="upload-row"><input type="file" accept="image/jpeg,image/png,image/webp,image/avif" data-upload-path="${encodePath(path)}"><button class="small" type="button" data-action="upload" data-path="${encodePath(path)}">Bild hochladen</button></div><div class="help">JPG, PNG, WebP oder AVIF; wird vor dem Upload automatisch fürs Web optimiert.</div>`;
}
...
async function uploadImage(button, path) {
  ...
  if (file.size > 20 * 1024 * 1024) return showToast('Das Ausgangsbild ist größer als 20 MB.');
  setLoading(true);
  try {
    ...
    const parent = path.slice(0, -1).reduce((item, key) => item?.[key], content);
    if (path.at(-1) === 'src' && parent && typeof parent === 'object') parent._full = result.path;
    markDirty();
    renderActiveTab();
...
async function optimizeImage(file) {
  ...
  if (!blob) throw new Error('Das Bild konnte nicht optimiert werden.');
...
function newItemForPath(path) {
  ...
  if (key === 'gallery') return { src: '', _full: '', alt_de: '', label_de: '', label_en: '', label_uk: '' };
