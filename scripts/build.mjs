...
async function validateImages(gallery) {
  for (const item of gallery) {
    ...
    if (!src) throw new Error(`Ungültiger Galeriepfad: ${item.src || '(leer)'}`);
    try {
...
function renderGallery(items) {
  return items.map((item, index) => {
    ...
    return `<button type="button" class="gi gi-${index}" onclick="openLb(${index})" aria-label="${attr(item.alt_de || item.label_de || `Galeriebild ${index + 1}`)}"><img src="${attr(safeAssetPath(item.src))}" alt="${attr(item.alt_de || item.label_de || '')}" loading="lazy" decoding="async"><span class="gi-ov"></span>${labels}</button>`;
  }).join('\n      ');
}
...
function renderStructuredData(source, data) {
  return source.replace(/<script type="application\/ld\+json">[\s\S]*?<\/script>/, () => {
    const schema = {
      ...
      image: (data.gallery || []).slice(0, 6).map(item => `${String(data.site?.canonical_url || '').replace(/\/$/, '')}/${safeAssetPath(item.src)}`),
...
function safeAssetPath(value) {
  ...
  if (!/^images\/[A-Za-z0-9À-žА-яІіЇїЄєҐґ _.,()&'-]+\.(?:jpe?g|png|webp|avif)$/i.test(normalized)) return '';
